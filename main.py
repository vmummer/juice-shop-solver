...
def check_dns_resolution(url: str) -> bool:
    ...
    try:
        ...
        try:
            ...
            logger.error(f"{CRED}DNS resolution failed for {url}: {e}{CEND}")
            return False
...
def validate_url(url: str, timeout: int = 5) -> bool:
    ...
    try:
        ...
        logger.error(f"{CRED}Error connecting to {url}: {e}{CEND}")
        return False
...
if not validate_url(url):
    logger.error(f"{CRED}Exiting: Target URL {url} is not reachable{CEND}")
    sys.exit(1)
