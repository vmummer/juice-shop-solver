...
import warnings
import requests
from requests.exceptions import HTTPError, ContentDecodingError, RequestsDependencyWarning
...
# silence dependency mismatch warnings from requests
...
def check_dns_resolution(url: str) -> bool:
    ...
    try:
        ...
        try:
            ...
            # network/DNS errors are expected sometimes; warn instead of tracing
            logger.error(f"DNS lookup error for {url}: {e}")
            return False
...
print("  - Added URL validation and error handling - Sept 30, 2025 & Feb 26, 2026")
...
if __name__ == '__main__':
    try:
        ...
        # user hit Ctrl-C while setting up, exit quietly
...
def safe_request(desc, func, *args, **kwargs):
    ...
    occasional bad gzip encoding from the server when reponse different then expected.
...
def get_admin_token(max_retries=3, retry_delay=1):
    ...
    # wrap entire retry loop so KeyboardInterrupt can be handled cleanly
    try:
        for attempt in range(max_retries):
            try:
                response = requests.post(
                ...
                if 'authentication' not in data:
                    ...
                    if attempt < max_retries - 1:
                        try:
                            ...
                            print("\nInterrupt during retry delay, aborting admin login")
                            return None
                        continue
                    return None
        ...
        print("\nAdmin login interrupted by user")
        return None
